...
housekeepingRouter.get("/login", (_req, res) => {
  ...
  <body style="font-family:system-ui;background:#f8fafc;display:flex;min-height:100vh;align-items:center;justify-content:center;margin:0">
    ...
    <style>.password-reveal-wrap{position:relative;display:inline-block;max-width:100%;vertical-align:top}.password-reveal-wrap>input{padding-right:42px!important}.password-reveal-btn{position:absolute;right:8px;top:50%;transform:translateY(-50%);border:0;background:transparent;color:#475569;cursor:pointer;width:28px;height:28px;border-radius:999px;font-size:16px;line-height:28px;text-align:center}.password-reveal-btn:hover,.password-reveal-btn:focus{background:#eef2f7;outline:0;color:#0f172a}</style>
    <script>(function(){function e(input){if(!input||input.dataset.passwordRevealBound==="1")return;input.dataset.passwordRevealBound="1";var parent=input.parentNode;if(!parent)return;var wrap=document.createElement("span");wrap.className="password-reveal-wrap";wrap.style.width=input.style.width||"100%";parent.insertBefore(wrap,input);wrap.appendChild(input);if(!input.style.width||input.style.width==="100%")input.style.width="100%";var btn=document.createElement("button");btn.type="button";btn.className="password-reveal-btn";btn.setAttribute("aria-label","Show password");btn.setAttribute("title","Show password");btn.innerHTML="&#128065;";btn.addEventListener("click",function(){var show=input.type==="password";input.type=show?"text":"password";btn.setAttribute("aria-label",show?"Hide password":"Show password");btn.setAttribute("title",show?"Hide password":"Show password");input.focus({preventScroll:true});});wrap.appendChild(btn);}function boot(){document.querySelectorAll('input[type="password"]').forEach(e);}if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",boot);else boot();})();</script>
  </body></html>`;
